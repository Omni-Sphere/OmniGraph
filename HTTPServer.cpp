...
void HTTPServer::handle_upload(const http::request<http::string_body> &req,
...
void HTTPServer::handle_download(const http::request<http::string_body> &req,
                                 std::shared_ptr<tcp::socket> socket) {
...
void HTTPServer::loadConfiguration() {
  if (!database) {
    std::cerr
        << "[WARNING] Database not set for HTTPServer. Using default CDN path: "
        << imgStoragePath << "\n";
    return;
  }
  ...
  try {
    ...
    Logger::Log("Failed to load configuration: " + std::string(e.what()) +
                    ". Using defaults.",
                LogType::ERROR);
  }
