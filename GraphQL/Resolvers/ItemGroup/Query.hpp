...
public:
  ...
  getGetItemGroup(
    ...
    try {
      omnisphere::omnierp::dtos::GetItemGroup getItemGroupDto{.Code = ItemGroupArg.Code,
                                                  .Name = ItemGroupArg.Name};

      omnisphere::omnierp::models::ItemGroup itemGroupModel =
          m_services->ItemGroup->Get(getItemGroupDto);
      ...
      throw graphql::service::schema_exception({graphql::service::schema_error{
          .message = e.what(), .path = {"ItemGroup", "GetItemGroup"}}});
    }
  }

  graphql::service::AwaitableObject<
      std::vector<std::shared_ptr<graphql::omnisphere::object::ItemGroup>>>
  getGetAllItemGroups(graphql::service::FieldParams &&params) const {
    try {
      std::vector<std::shared_ptr<graphql::omnisphere::object::ItemGroup>>
          ItemGroups;

      std::vector<omnisphere::omnierp::models::ItemGroup> itemGroupModel =
          m_services->ItemGroup->GetAll();

      for (const auto &itemGroup : itemGroupModel) {
        auto resolver = std::make_shared<ItemGroupResolver>(
            std::make_shared<omnisphere::omnierp::models::ItemGroup>(itemGroup),
            m_services);

        ItemGroups.push_back(
            std::make_shared<graphql::omnisphere::object::ItemGroup>(
                std::move(resolver)));
      }
      ...
      throw graphql::service::schema_exception({graphql::service::schema_error{
          .message = e.what(), .path = {"ItemGroup", "GetAllItemGroups"}}});
    }
  }
