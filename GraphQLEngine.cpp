...
GraphQLEngine::GraphQLEngine(std::shared_ptr<omnicore::service::Database> db) : database(std::move(db))
{
    std::system("clear");
...
{
    using namespace graphql::response;
    ...
    case Type::Boolean:
        return v.get<BooleanType>();

    case Type::Int:
        return v.get<IntType>();

    case Type::Float:
        return v.get<FloatType>();

    case Type::String:
    case Type::EnumValue:
    case Type::ID:
        return boost::json::string(v.get<StringType>());

    case Type::List:
    {
        boost::json::array arr;

        for (auto &elem : v.get<ListType>())
        {
...
boost::json::object GraphQLEngine::astNodeToJson(const graphql::peg::ast_node *node)
{
...
http::response<http::string_body> GraphQLEngine::handleGraphQLRequest(const http::request<http::string_body> &req)
{
    try
    {
        ...
        std::shared_ptr<ServiceInyector> serviceInyector = std::make_shared<ServiceInyector>(database);

        std::shared_ptr<graphql::resolvers::RootQueryResolver> rootQ = std::make_shared<graphql::resolvers::RootQueryResolver>(serviceInyector);
        std::shared_ptr<graphql::resolvers::RootMutationResolver> rootM = std::make_shared<graphql::resolvers::RootMutationResolver>(serviceInyector);

        std::shared_ptr<graphql::omnisphere::object::Query> qObj = std::make_shared<graphql::omnisphere::object::Query>(rootQ);
        std::shared_ptr<graphql::omnisphere::object::Mutation> mObj = std::make_shared<graphql::omnisphere::object::Mutation>(rootM);

        request = std::make_shared<graphql::omnisphere::Operations>(
            qObj,
            mObj);
